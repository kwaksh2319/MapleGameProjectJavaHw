...
class  mapletest04 extends JFrame implements KeyListener{
	int huntf=0;
	int tmpr=0;
	int tmpl=0;
	int rightw=0;
	...
	private Timer m_timer01=new Timer();
...
mapletest04(int j, String st,int mp1,int hp1,int x1,int y1,int movestatus1,int level1,double hunt1 ){
	...
	x=x1;
	y=y1;
	moveStatus=movestatus1;
    setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
    
	this.setTitle("maplemade4");	
	...
	jbuttontest();
	if(keyUp==false&&x>70){
	frame04.schedule(frame04_1, 1,100);
	}
	
	}
private Timer frame04=new Timer();

private TimerTask frame04_1=new TimerTask(){
...
public void keyPressed(KeyEvent e){
	...
	case KeyEvent.VK_UP:
		...
		if(x<=70){
			...
			this.dispose();
			this.remove(this);
			mapletest03 mp3 = new mapletest03(c, str,mp,hp,800,y,2,level,hunt);
		...
		if(x>=800){
			...
			this.dispose();
			this.remove(this);
			mapletest05 mp5 = new mapletest05(c, str,mp,hp,70,y,moveStatus,level,hunt);
...
public void jbuttontest(){
	...
	public void paint (Graphics g){
		...
		Image img = Toolkit.getDefaultToolkit().getImage("map_theif.png");//배경이미지 삽입 
		
		Image img01 = Toolkit.getDefaultToolkit().getImage("statebar.png");//배경이미지 삽입 
		g.drawImage(img, 0, 60, 880, 610,this);// 이미지 수정
		...
		g.fillRect(376, 650, mpp, 12);
		 ...
		 if(monsterhp01==0&&huntf==0&&level==1&&level01<1){
			 hunt=hunt+113;
			 huntf=1;
			 
		 }
		 else if(monsterhp01==0&&huntf==0&&level==2&&level01<1){
			 hunt=hunt+50;
			 huntf=1;
			 
		 }
		 else if(monsterhp01==0&&huntf==0&&level==3&&level01<1){
			 hunt=hunt+30;
			 huntf=1;
			 
		 }
		 else if(monsterhp01==0&&huntf==0&&level==4&&level01<1){
			 hunt=hunt+30;
			 huntf=1;
			 
		 }
		 else if(monsterhp01==0&&huntf==0&&(level>=5||level01>=1)){
			 hunt=hunt+5;
			 huntf=1;
		...
		g.drawString(str, 107, 655);
		 ...
		 g.drawImage(portal[0], 50, 500, 80, 100,this);
	      ...
	      monster01_level01_move_left[0]= Toolkit.getDefaultToolkit().getImage("monster//monster_level01.gif");
	      monster01_level01_move_left[1]= Toolkit.getDefaultToolkit().getImage("monster//monster_level01_right.gif");
			...
			TimerTask task01=new TimerTask(){
				...
				/*	if(monsterhp01==0){
					...
					if(x!=monsterx01&&(moveStatus==0||moveStatus==1))
					{  if(monsterx01>80){
						monsterx01=monsterx01-0.001;
					...
					else if(x!=monsterx01&&moveStatus==2)
					{   if(monsterx01<750){
						monsterx01=monsterx01+0.001;} 
			...
			if(monsterhp01>0){
			m_timer01.schedule(task01, 0,100);
			}
			...
			//	 monsterx01=monsterx01+0.1;
				...
				if(monsterhp01>0){
					 if(x<=monsterx01&&x+30>=monsterx01&&hp>=0){
						...
						g.drawImage(monster01_level01_move_left[0], (int)monsterx01, 535, 60, 60,this);}
		...
		//	 monsterx01=monsterx01+0.1;
			...
			if(monsterhp01>0){
				 if(x<=monsterx01&&x-30>=monsterx01&&hp>=0){
					...
					g.drawImage(monster01_level01_move_left[1], (int)monsterx01, 535, 60, 60,this);}
...
if(char_jump/4>2){
	...
	if(char_jump/4>2){
		...
		g.drawImage(war_jump[char_jump/4], charx, y,  charwidth, 65,this);
			...
			else if(Attack==true&&keyLeft==false&&Skill01==false&&Skill02==false&&Skill03==false&&Skill04==false){
				if(moveStatus==2){
