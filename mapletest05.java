...
class  mapletest05 extends JFrame implements KeyListener{
	int huntf=0;
	int tmpr=0;
	int tmpl=0;
	int rightw=0;
...
mapletest05(int j, String st,int mp1,int hp1,int x1,int y1,int movestaus1,int level1,double hunt1 ){
	...
	jbuttontest();
	if(keyUp==false&&x>70){
	frame05.schedule(frame05_1, 1,100);
	}
	
	}
private Timer frame05=new Timer();

private TimerTask frame05_1=new TimerTask(){
...
public void keyPressed(KeyEvent e){
	...
	case KeyEvent.VK_UP:
		...
		if(x>=800){
			...
			this.dispose();
			this.remove(this);
			mapletest06 mp6 = new mapletest06(c, str,mp,hp,70,y,moveStatus,level,hunt);
...
public void jbuttontest(){
	...
	public void paint (Graphics g){
		...
		Image img = Toolkit.getDefaultToolkit().getImage("map02.png");//배경이미지 삽입 
		
		Image img01 = Toolkit.getDefaultToolkit().getImage("statebar.png");//배경이미지 삽입 
		g.drawImage(img, 0, 120, 880, 580,this);// 이미지 수정
		...
		if(hunt>=113){
		 ...
		 if(monsterhp01==0&&huntf==0&&level==4&&level01<1){
			 hunt=hunt+113;
			 huntf=1;
		 ...
		 else if(monsterhp01==0&&huntf==0&&level==6&&level01<1){
			 hunt=hunt+30;
			 huntf=1;
			 
		 }
		 else if(monsterhp01==0&&huntf==0&&level==7&&level01<1){
			 hunt=hunt+30;
			 huntf=1;
			 
		 }
		 else if(monsterhp01==0&&huntf==0&&(level>=8||level01>=1)){
			 hunt=hunt+5;
			 huntf=1;
		...
		g.drawString(str, 107, 655);
		 ...
		 g.drawImage(portal[0], 50, 500, 80, 100,this);
	      ...
	      monster01_level01_move_left[0]= Toolkit.getDefaultToolkit().getImage("monster//monster_level02_left.gif");
	      monster01_level01_move_left[1]= Toolkit.getDefaultToolkit().getImage("monster//monster_level02_right.gif");
			...
			//	 monsterx01=monsterx01+0.1;
				...
				if(monsterhp01>0){
					 if(x<=monsterx01&&x+30>=monsterx01&&hp>=0){
						...
						g.drawImage(monster01_level01_move_left[0], (int)monsterx01, 490, 100, 100,this);}
		...
		//	 monsterx01=monsterx01+0.1;
			...
			if(monsterhp01>0){
				 if(x<=monsterx01&&x-30>=monsterx01&&hp>=0){
					...
					g.drawImage(monster01_level01_move_left[1], (int)monsterx01, 490, 100, 100,this);}
